<template>
  ...
  <ul class="side-bar" :class="fold">
      <div class="sidebar" :class="fold">
      <p class="sidebar-text">{{ bartitle }}</p>
    </div>
    <div>
      <li style="margin-top: 24px;width: 24px;"  @click="toggleFold">

        <svg class="icon" viewBox="0 0 1024 1024" fill="currentColor" aria-hidden="true">
          <path xmlns="http://www.w3.org/2000/svg"
            style=" stroke:none;fill-rule:nonzero;fill:rgb(0%,0%,0%);fill-opacity:1;"
            d="M63.7 191.4h895.2v64H63.7zM63.7 480.2h895.2v64H63.7zM63.7 767.2h895.2v64H63.7z" />
        </svg>
        <span class="label" :class="fold"></span>
      </li>
      <li @click="toggleFold" class="search-fold" :class="fold">
      ...
      <div style="width: var(--sb-width, 350px);margin: auto;">
        <wininputbox placeholder="查找" class="inputbox" style="margin-top: 6px;width: 320px;transition: all 0s ease;" :class="fold"/>
      </div>
      <h3 style="margin-top: 26px;margin-bottom: 26px;margin-left: 18px;" class="title" :class="fold">{{ title }}</h3>
    </div>
    <div style="overflow: auto;height: calc( 100% - 240px );overflow-x: hidden;">
      <li v-for="(item, idx) in items" :key="idx" :class="{ active: idx === modelValue }"
        ...
        <svg class="icon" viewBox="0 0 1024 1024" fill="currentColor" aria-hidden="true" height="16">
          <path :d="item.icon" />
        </svg>

        <span class="label" :class="fold">{{ item.name }}</span>
      </li>
    </div>
  </ul>
</template>
